...
def default_instanceID():
    status, output = subprocess.getstatusoutput("curl -s -H 'Metadata-Flavor:Google' 'http://metadata.google.internal/computeMetadata/v1/instance/id'")
    instanceID = "unknown"
    if status == 0:
...
def default_publicIP():
    status, output = subprocess.getstatusoutput("curl -s -H 'Metadata-Flavor:Google' 'http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/0/access-configs/0/external-ip'")
    publicIP = "unknown"
    if status == 0:
